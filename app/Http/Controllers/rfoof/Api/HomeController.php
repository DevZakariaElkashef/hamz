...
class HomeController extends Controller
{
    ...
    {
        try {
            $categories = CategoryResource::collection(Category::rfoof()->get())->toArray(request());
            $data = [];
            $data[0] = [
                'id' => 0,
    ...
    {
        try {
            $categories = CategoryResource::collection(Category::rfoof()->whereIn('id', [3, 4, 5, 6])->get());
    ...
    {
        try {
            $subCategories = SubCategoryResource::collection(SubCategory::rfoof()->where('category_id', $request->category_id)->get());
    ...
    {
        try {
            $marka = MarkaResource::collection(Marka::rfoof()->get());
            $models = ModelResource::collection(ModelTypes::rfoof()->get());
            $types = TypeResource::collection(Type::rfoof()->get());
            $colors = ColorResource::collection(Color::rfoof()->get());
            $fuelTypes = FuelTypeResource::collection(FuelType::rfoof()->get());
            $productStatus = ProductStatusResource::collection(ProductStatus::rfoof()->get());
            $carPush = CarPushResource::collection(CarPush::rfoof()->get());
            $directions = DirectionResource::collection(Direction::rfoof()->get());
    ...
    public function getModels(Request $request)
    {
        try {
            if ($request->category_id) {
                $models = ModelResource::collection(ModelTypes::rfoof()->where('category_id', $request->category_id)->get());
            } else {
                $models = ModelResource::collection(ModelTypes::rfoof()->where('sub_category_id', $request->sub_category_id)->get());
            }
    ...
    public function filter(Request $request)
    {
        try {
            ...
            $products = $products->rfoof()->active()->where([
                'status' => 1,
                'verify' => 1
            ]);
