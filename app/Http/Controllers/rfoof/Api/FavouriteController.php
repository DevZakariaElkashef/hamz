...
class FavouriteController extends Controller
{
    ...
    {
        try{
            $products = ProductResource::collection(Product::rfoof()->whereHas('favourities', function ($query) use($request){
    ...
    {
        try{
            $favourite = Favourite::rfoof()->where(['product_id' => $request->product_id, 'user_id' => $request->user()->id])->first();
            if($favourite)
            {
                $favourite->delete();
