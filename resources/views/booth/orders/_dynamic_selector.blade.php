<script>
    ...
    $(document).on('change', '#sectionId', async function() {
        ...
        await fetchData("{{ route('booth.stores.bySection') }}", {
            sectionId: sectionId
        }, '#storeId');
        await fetchData("{{ route('booth.categories.bySection') }}", {
            sectionId: sectionId
        }, '#categoryId');
        await fetchData("{{ route('booth.brands.bySection') }}", {
            sectionId: sectionId
        }, '#brandId');
    });
    ...
    $(document).on('change', '#storeId', async function() {
        ...
        await fetchData("{{ route('booth.categories.byStore') }}", {
            storeId: storeId
        }, '#categoryId');
        await fetchData("{{ route('booth.brands.byStore') }}", {
            storeId: storeId
        }, '#brandId');
    });
</script>
