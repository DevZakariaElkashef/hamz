    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ <a class="text-dark"
                        href="{{ route('booth.coupons.edit', $coupon->id) }}">{{ __('main.edit_coupon') }}</a></span>
            </div>
        </div>
    ...
    <div class="row row-sm">
        <div class="col-xl-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="post" action="{{ route('booth.coupons.update', $coupon->id) }}" data-parsley-validate=""
                                ...
                                <select required class="form-control" name="store_id">
                                    @foreach ($stores as $store)
                                        <option value="{{ $store->id }}"
                                            @if (old('store_id') == $coupon->store_id) selected @endif>
                                            {{ $store->name }}</option>
                                    @endforeach
                                </select>
                                @error('store_id')
