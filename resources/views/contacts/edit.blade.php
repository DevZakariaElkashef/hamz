    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ <a class="text-dark"
                        href="{{ route('contacts.edit', $contact->id) }}">{{ __('main.edit_contact') }}</a></span>
            </div>
        </div>
        <div class="d-flex my-xl-auto right-content">
            <div class="mb-3 mb-xl-0">
                <a href="{{ route('contacts.index') }}" class="btn btn-secondary ">{{ __('main.back') }}</a>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
    ...
    <div class="row row-sm">
        <div class="col-xl-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="post" action="{{ route('contacts.update', $contact->id) }}" data-parsley-validate=""
                        ...
                        <div class="row">
                            <div class="col-md-6 form-group mg-b-0">
                                ...
                                <input class="form-control" name="name_ar" placeholder="{{ __('main.enter_name') }}"
                                    required="" type="text" value="{{ old('name_ar') ?? $contact->name_ar }}">
                                @error('name_ar')
                            ...
                            <div class="col-md-6 form-group mg-b-0">
                                ...
                                <input class="form-control" name="name_en" placeholder="{{ __('main.enter_name') }}"
                                    required="" type="text" value="{{ old('name_en') ?? $contact->name_en }}">
                                @error('name_en')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 form-group mg-b-0">
                                ...
                                <input class="form-control" name="url" placeholder="{{ __('main.enter_name') }}"
                                    type="url" value="{{ old('url') ?? $contact->url }}">
                                @error('url')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 form-group mg-b-0">
                                ...
                                <select required class="form-control" name="is_active">
                                    <option value="0" @if (old('is_active', $contact->is_active) == 0) selected @endif>
                                        {{ __('main.not_active') }}</option>
                                    <option value="1" @if (old('is_active', $contact->is_active) == 1) selected @endif>
