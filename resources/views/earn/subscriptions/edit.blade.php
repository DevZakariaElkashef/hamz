        ...
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ <a class="text-dark"
                        href="{{ route('earn.subscriptions.index') }}">{{ __('main.subscriptions') }}</a></span>
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ <a class="text-dark"
                        href="{{ route('earn.subscriptions.edit', $subscription->id) }}">{{ __('main.edit_subscription') }}</a></span>
            </div>
        </div>
        <div class="d-flex my-xl-auto right-content">
            <div class="mb-3 mb-xl-0">
                <a href="{{ route('earn.subscriptions.index') }}" class="btn btn-secondary ">{{ __('main.back') }}</a>
            </div>
        </div>
    </div>
    <!-- breadcrumb -->
            ...
            <div class="card">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between">
                        <h4 class="card-title mg-b-0">{{ __('main.subscriptions') }}</h4>
                    </div>
                </div>
                <div class="card-body">
                    <form method="post" action="{{ route('earn.subscriptions.update', $subscription->id) }}" data-parsley-validate=""
                        ...
                        <div class="row">
                            <div class="col-md-6 form-group mg-b-0">
                                ...
                                <input class="form-control" name="name_ar" placeholder="{{ __('main.enter_name') }}"
                                    required="" type="text" value="{{ old('name_ar') ?? $subscription->name_ar }}">
                                @error('name_ar')
                            ...
                            <div class="col-md-6 form-group mg-b-0">
                                ...
                                <input class="form-control" name="name_en" placeholder="{{ __('main.enter_name') }}"
                                    required="" type="text" value="{{ old('name_en') ?? $subscription->name_en }}">
                                @error('name_en')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 form-group mg-b-0">
                                ...
                                <input class="form-control" name="url" placeholder="{{ __('main.enter_name') }}"
                                    type="url" value="{{ old('url') ?? $subscription->url }}">
                                @error('url')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6 form-group mg-b-0">
                                ...
                                <select required class="form-control" name="is_active">
                                    <option value="0" @if (old('is_active', $subscription->is_active) == 0) selected @endif>
                                        {{ __('main.not_active') }}</option>
                                    <option value="1" @if (old('is_active', $subscription->is_active) == 1) selected @endif>
