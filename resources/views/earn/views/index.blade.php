...
@section('page-header')
    ...
    <div class="breadcrumb-header justify-content-between">
        <div class="my-auto">
            <div class="d-flex">
                ...
                <span class="text-muted mt-1 tx-13 mr-2 mb-0">/ <a class="text-dark"
                        href="{{ route('earn.views.index') }}">{{ __('main.views') }}</a></span>
            </div>
        </div>
        <div class="d-flex my-xl-auto right-content">
            ...
            <div class="pr-1 mb-3 mb-xl-0">
                <a href="{{ route('earn.views.index') }}" class="btn btn-warning  btn-icon ml-2"><i
    ...
    <div class="modal" id="filterModal">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content modal-content-demo">
                ...
                <form method="get" action="{{ route('earn.views.index') }}">
                    <div class="modal-body">
                        ...
                        <div class="form-group">
                            <label for="status">{{ __('main.is_watched') }}</label>
                            <select name="status" class="form-control">
                                <option value="">{{ __('main.all') }}</option>
                                <option value="0" @if(request('status') == '0') selected @endif>{{ __('main.not_watched') }}</option>
                                <option value="1" @if(request('status') == '1') selected @endif>{{ __('main.watched') }}</option>
                            </select>
                        </div>
                    </div>
                    <div class="modal-footer">
            ...
            <div class="card">
                <div class="card-header pb-0">
                    <div class="d-flex justify-content-between">
                        <h4 class="card-title mg-b-0">{{ __('main.views') }}</h4>
                        <div class="d-flex align-items-center">
                            <a href="#" class="btn btn-danger mx-1 d-none" id="deleteSelectionBtn"
                                ...
                                data-url="{{ route('earn.views.delete') }}">{{ __('main.delete') }}</a>

                            <input type="text" id="searchInput" data-url="{{ route('earn.views.search') }}"
                            ...
                            <div class="custom-select-wrapper mx-1">
                                <select id="showPerPage" class="custom-select"
                                    data-url="{{ route('earn.views.index') }}" onchange="updatePageSize()">
