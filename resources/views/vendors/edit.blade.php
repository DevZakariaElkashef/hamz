    ...
    <div class="row row-sm">
        <div class="col-xl-12">
            <div class="card">
                ...
                <div class="card-body">
                    <form method="post" action="{{ route('vendors.update', $vendor->id) }}" data-parsley-validate=""
                        ...
                        <div class="row">
                            <div class="col-md-6 form-group mg-b-0">
                                ...
                                <input class="form-control" name="name" placeholder="{{ __('main.enter_name') }}"
                                    required="" type="text" value="{{ old('name') ?? $vendor->name }}">
                                @error('name')
                            ...
                            <div class="col-md-6 form-group mg-b-0">
                                ...
                                <input class="form-control" name="email" placeholder="{{ __('main.enter_mail') }}"
                                    type="email" value="{{ old('email') ?? $vendor->email }}">
                                @error('email')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="col-md-6 form-group mg-b-0">
                                ...
                                <input class="form-control" name="phone" placeholder="{{ __('main.enter_phone') }}"
                                    required="" type="text" value="{{ old('phone') ?? $vendor->phone }}">
                                @error('phone')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                                ...
                                <select required class="form-control select2" name="city_id">
                                    @foreach ($cities as $city)
                                        <option value="{{ $city->id }}"
                                            @if (old('city_id', $vendor->city_id) == $city->id) selected @endif>{{ $city->name }}</option>
                                    @endforeach
                                </select>
                                @error('city_id')
                                    <div class="text-danger">{{ $message }}</div>
                            ...
                            <div class="col-md-6 form-group mg-b-0">
                                ...
                                <select required class="form-control" name="is_active">
                                    <option value="0" @if (old('is_active', $vendor->is_active) == 0) selected @endif>
                                        {{ __('main.not_active') }}</option>
                                    <option value="1" @if (old('is_active', $vendor->is_active) == 1) selected @endif>
